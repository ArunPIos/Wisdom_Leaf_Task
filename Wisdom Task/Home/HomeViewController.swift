...
class HomeViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        searchBar.tintColor = #colorLiteral(red: 0.8745098039, green: 0.2588235294, blue: 0.2549019608, alpha: 1)
        searchBar.searchTextField.textColor = #colorLiteral(red: 0.8745098039, green: 0.2588235294, blue: 0.2549019608, alpha: 1)
    ...
    private func setupViewModel() {
        self.homeVM.fetchFilter(searchString: "Alpha") { response in
            switch response {
            case .success(_) :
...
extension HomeViewController:  UITableViewDataSource,  UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let imageData = movie.movieImage {
            ...
            self.homeVM.fetchImage(for: indexPath.row) { [weak cell] image
                in DispatchQueue
                    .main.async {
